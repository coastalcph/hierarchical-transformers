...
def test_memory_usage(model, steps=20, batch_size=2, seq_length=4096, task_type='lm'):
    ...
    if task_type != 'mc_qa':
        ...
        input_ids = torch.randint(1, 40000, (batch_size, 5, seq_length), dtype=torch.long).to('cuda')
        input_ids[:, :: 128] = model.config.bos_token_id
        # labels = input_ids.clone()
        attention_mask = torch.ones((batch_size, 5, seq_length), dtype=torch.int).to('cuda')
...
def estimate_model_size():
    for task in ['lm', 'doc_cls', 'mc_qa']:
        ...
        memory_use, time_use = test_memory_usage(htf_model, seq_length=lf_config.model_max_length, task_type=task)
        ...
        for layout in LAYOUTS:
            ...
            model_total_params = model_total_params / 1e6
            memory_use, time_use = test_memory_usage(htf_model, seq_length=int(MAX_SENTENCE_LENGTH * MAX_SENTENCES), task_type=task)
