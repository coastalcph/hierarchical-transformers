...
def convert_roberta_to_htf():
    ...
    htf_tokenizer = HATTokenizer.from_pretrained(f'{DATA_DIR}/PLMs/hat-{config.layout}-roberta')
    print(f'RoBERTa-based HAT model with layout {config.layout} is ready to run!')
