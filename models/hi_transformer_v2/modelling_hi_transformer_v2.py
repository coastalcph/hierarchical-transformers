...
"""PyTorch Hi-Transformer V2 model."""
...
from models.hi_transformer_v2.configuration_hi_transformer_v2 import HiTransformerV2Config
from models.hi_transformer.modelling_hi_transformer import TransformerLayer, BaseModelOutputWithSentenceAttentions, \
    HiTransformerEmbeddings, HiTransformerPooler
from models.hi_transformer.modelling_hi_transformer import transform_tokens2sentences, transform_masks2sentences, \
    transform_sentences2tokens
from transformers.activations import gelu


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "hi-transformer-v2-base"
_CONFIG_FOR_DOC = "HiTransformerConfigV2"
_TOKENIZER_FOR_DOC = "HiTransformerTokenizerV2"

HITRANSFORMERV2_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "hi-transformer-v2-base",
    "hi-transformer-v2-large",
...
class HiTransformerV2Encoder(nn.Module):
...
class HiTransformerV2PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = HiTransformerV2Config
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, HiTransformerV2Encoder):
...
HITRANSFORMER_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`HiTransformerV2Config`]): Model configuration class with all the parameters of the
...
HITRANSFORMER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            ...
            Indices can be obtained using [`HiTransformerV2Tokenizer`]. See [`PreTrainedTokenizer.encode`] and
...
class HiTransformerV2Model(HiTransformerV2PreTrainedModel):
    """
    ...
    # Copied from transformers.models.bert.modeling_bert.BertModel.__init__ with Bert->HiTransformerV2
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = HiTransformerEmbeddings(config)
        self.encoder = HiTransformerV2Encoder(config)
...
class HiTransformeLMHead(nn.Module):
    """HiTransformerV2 Head for masked language modeling."""
...
class HiTransformerV2ForMaskedLM(HiTransformerV2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.hi_transformer = HiTransformerV2Model(config)
        self.lm_head = HiTransformeLMHead(config, sampled_decoding=False)
...
class HiTransformerV2ForSequenceClassification(HiTransformerV2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
@add_start_docstrings(
    """
    HiTransformerV2 Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
    softmax) e.g. for RocStories/SWAG tasks.
    """,
    HITRANSFORMER_START_DOCSTRING,
)
class HiTransformerV2ForMultipleChoice(HiTransformerV2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
@add_start_docstrings(
    """
    HiTransformerV2 Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """,
    HITRANSFORMER_START_DOCSTRING,
)
class HiTransformerV2ForTokenClassification(HiTransformerV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.hi_transformer = HiTransformerV2Model(config, add_pooling_layer=False)
        classifier_dropout = (
...
@add_start_docstrings(
    """
    HiTransformerV2 Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layers on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    HITRANSFORMER_START_DOCSTRING,
)
class HiTransformerV2ForQuestionAnswering(HiTransformerV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
