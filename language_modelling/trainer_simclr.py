...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        sent_contr_loss = torch.tensor(0.0).to(args.device)
        sent_std_loss = torch.tensor(0.0).to(args.device)
        sent_cov_loss = torch.tensor(0.0).to(args.device)
        doc_contr_loss = torch.tensor(0.0).to(args.device)
        ...
        self._sent_contr_loss_scalar = 0.0
        self._sent_std_loss_scalar = 0.0
        self._sent_cov_loss_scalar = 0.0
        self._doc_contr_loss_scalar = 0.0
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, mlm_loss_step, sent_contr_loss_step, sent_std_loss_step, sent_cov_loss_step, \
                        doc_contr_loss_step, doc_std_loss_step, doc_cov_loss_step = self.training_step(model, inputs)
                else:
                    tr_loss_step, mlm_loss_step, sent_contr_loss_step, sent_std_loss_step, sent_cov_loss_step, \
                    doc_contr_loss_step, doc_std_loss_step, doc_cov_loss_step = self.training_step(model, inputs)
                ...
                if (
                    ...
                    sent_contr_loss += sent_contr_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                    ...
                    doc_contr_loss += doc_contr_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                    ...
                    sent_contr_loss += sent_contr_loss_step
                    sent_std_loss += sent_std_loss_step
                    sent_cov_loss += sent_cov_loss_step
                    doc_contr_loss += doc_contr_loss_step
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss,
                                                  doc_contr_loss, doc_std_loss, doc_cov_loss, model, trial, epoch,
        ...
        self._sent_contr_loss_scalar += sent_contr_loss.item()
        self._sent_std_loss_scalar += sent_std_loss.item()
        self._sent_cov_loss_scalar += sent_cov_loss.item()
        self._doc_contr_loss_scalar += doc_contr_loss.item()
        ...
        train_sent_contr_loss = self._sent_contr_loss_scalar / self.state.global_step
        ...
        train_doc_contr_loss = self._doc_contr_loss_scalar / self.state.global_step
        ...
        if train_sent_contr_loss != 0:
            metrics["train_sent_contr_loss"] = train_sent_contr_loss
        ...
        if train_doc_contr_loss != 0:
            metrics["train_doc_contr_loss"] = train_doc_contr_loss
    ...
    def _maybe_log_save_evaluate(self, tr_loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss,
                                 doc_contr_loss, doc_std_loss, doc_cov_loss, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            sent_contr_loss_loss_scalar = self._nested_gather(sent_contr_loss).mean().item()
            ...
            doc_contr_loss_loss_scalar = self._nested_gather(doc_contr_loss).mean().item()
            ...
            if sent_contr_loss_loss_scalar != 0:
                logs["sent_contr_loss"] = round(sent_contr_loss_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            ...
            if doc_contr_loss_loss_scalar != 0:
                logs["doc_contr_loss"] = round(doc_contr_loss_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.autocast_smart_context_manager():
            loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss, doc_contr_loss, doc_std_loss, doc_cov_loss \
                = self.compute_loss(model, inputs)

        if mlm_loss is None:
            mlm_loss = torch.tensor(0.0).to(loss.device)
        if sent_contr_loss is None:
            sent_contr_loss = torch.tensor(0.0).to(loss.device)
        ...
        if doc_contr_loss is None:
            doc_contr_loss = torch.tensor(0.0).to(loss.device)
        ...
        if self.args.n_gpu > 1:
            ...
            sent_contr_loss = sent_contr_loss.mean()
            sent_std_loss = sent_std_loss.mean()
            sent_cov_loss = sent_cov_loss.mean()
            doc_contr_loss = doc_contr_loss.mean()
        ...
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            ...
            sent_contr_loss = sent_contr_loss / self.args.gradient_accumulation_steps
            ...
            doc_contr_loss = doc_contr_loss / self.args.gradient_accumulation_steps
        ...
        return loss.detach(), mlm_loss.detach(), sent_contr_loss.detach(), sent_std_loss.detach(), sent_cov_loss.detach(), \
               doc_contr_loss.detach(), doc_std_loss.detach(), doc_cov_loss.detach()
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        return (loss, outputs.mlm_loss, outputs.sent_contr_loss, outputs.sent_std_loss, outputs.sent_cov_loss,
                outputs.doc_contr_loss, outputs.doc_std_loss, outputs.doc_cov_loss, outputs) if return_outputs \
                   else (loss, outputs.mlm_loss, outputs.sent_contr_loss, outputs.sent_std_loss, outputs.sent_cov_loss,
                         outputs.doc_contr_loss, outputs.doc_std_loss, outputs.doc_cov_loss)
    ...
    def evaluation_loop(
        ...
        sent_contr_losses_host = None
        sent_std_losses_host = None
        sent_cov_losses_host = None
        doc_contr_losses_host = None
        ...
        mlm_losses = None
        sent_contr_losses = None
        sent_std_losses = None
        sent_cov_losses = None
        doc_contr_losses = None
        doc_std_losses = None
        doc_cov_losses = None
        all_preds = None
        all_labels = None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss, doc_contr_loss, doc_std_loss, doc_cov_loss, \
            logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if sent_contr_loss is not None:
                sent_contr_losses = self._nested_gather(sent_contr_loss.repeat(batch_size))
                sent_contr_losses_host = sent_contr_losses if sent_contr_losses_host is None else torch.cat((sent_contr_losses_host, sent_contr_losses), dim=0)
            ...
            if doc_contr_loss is not None:
                doc_contr_losses = self._nested_gather(doc_contr_loss.repeat(batch_size))
                doc_contr_losses_host = doc_contr_losses if doc_contr_losses_host is None else torch.cat((doc_contr_losses_host, doc_contr_losses), dim=0)
        ...
        if sent_contr_losses_host is not None:
            sent_contr_losses = nested_numpify(sent_contr_losses_host)
            sent_sim_all_losses = sent_contr_losses if sent_sim_all_losses is None else np.concatenate((sent_sim_all_losses, sent_contr_losses), axis=0)
        ...
        if doc_contr_losses_host is not None:
            doc_contr_losses = nested_numpify(doc_contr_losses_host)
            doc_sim_all_losses = doc_contr_losses if doc_sim_all_losses is None else np.concatenate((doc_sim_all_losses, doc_contr_losses), axis=0)
        ...
        if sent_sim_all_losses is not None:
            metrics[f"{metric_key_prefix}_sent_contr_loss"] = sent_sim_all_losses.mean().item()
        if sent_std_all_losses is not None:
        ...
        if doc_sim_all_losses is not None:
            metrics[f"{metric_key_prefix}_doc_contr_loss"] = doc_sim_all_losses.mean().item()
        if doc_std_all_losses is not None:
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    with self.autocast_smart_context_manager():
                        loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss, doc_contr_loss, doc_std_loss, \
                        doc_cov_loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
                    loss = loss.mean().detach()
                    if mlm_loss is not None:
                        mlm_loss = mlm_loss.mean().detach()
                    if sent_contr_loss is not None:
                        sent_contr_loss = sent_contr_loss.mean().detach()
                    if sent_std_loss is not None:
                        sent_std_loss = sent_std_loss.mean().detach()
                    if sent_cov_loss is not None:
                        sent_cov_loss = sent_cov_loss.mean().detach()
                    if doc_contr_loss is not None:
                        doc_contr_loss = doc_contr_loss.mean().detach()
                    if doc_std_loss is not None:
        ...
        if prediction_loss_only:
            return (loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss,
                    doc_contr_loss, doc_std_loss, doc_cov_loss, None, None)
        ...
        return (loss, mlm_loss, sent_contr_loss, sent_std_loss, sent_cov_loss,
                doc_contr_loss, doc_std_loss, doc_cov_loss, logits, labels)
